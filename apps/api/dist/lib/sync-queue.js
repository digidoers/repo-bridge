...
function getFileContentFromRef(ref, safePath, cwd) {
    try {
...
function syncSingleFile(filePath, baseRef, commitSha, targetDir, tempDir, isDryRun, savedResolvedContent) {
    ...
    try {
        ...
        try {
            fs.rmSync(baseTmp, { force: true });
        }
        catch { }
        try {
            fs.rmSync(commitTmp, { force: true });
        }
        catch { }
    }
}
...
class SyncQueue {
    ...
    async runRealDryRun(job, options = {}) {
        ...
        try {
            ...
            if (job.pushEvent.baseSha === "HEAD") {
                ...
                selectedPaths = job.files.map((f) => f.filePath);
            }
            ...
            for (const jobFile of job.files) {
                ...
                const syncRes = syncSingleFile(jobFile.filePath, baseRef, job.pushEvent.commitSha, targetDir, tempDir, true // isDryRun = true
                );
    ...
    // ─── Apply Sync Jobs ──────────────────────────────────
    enqueueApply(syncJobId, options = {}) {
        setImmediate(async () => {
            await this.processApply(syncJobId, options);
        });
    }
    async resolveConflictFile(syncJobId, filePath, resolvedContent) {
        try {
            const job = await prisma.syncJob.findUnique({
                where: { id: syncJobId },
                include: {
            ...
            if (!job.files.some((file) => file.filePath === filePath && (file.mergeResult === "CONFLICT" || file.mergeResult === "MERGED"))) {
    ...
    async bulkResolveConflictFiles(syncJobId, filePaths, resolutions) {
        try {
            const job = await prisma.syncJob.findUnique({
                where: { id: syncJobId },
                include: { files: true },
    ...
    async resetConflictFile(syncJobId, filePath) {
        try {
            const job = await prisma.syncJob.findUnique({
                where: { id: syncJobId },
                include: {
            ...
            try {
                ...
                const baseRef = job.pushEvent.baseSha === "HEAD" ? `${job.pushEvent.commitSha}~1` : job.pushEvent.baseSha;
                const syncRes = syncSingleFile(filePath, baseRef, job.pushEvent.commitSha, targetDir, tempDir, true);
                await prisma.syncJobFile.updateMany({
                    where: {
                        syncJobId,
                        filePath,
    ...
    async processApply(syncJobId, options = {}) {
        if (this.activeJobs.has(syncJobId))
            return;
        this.activeJobs.add(syncJobId);
        ...
        try {
            ...
            const canMergeExistingPr = options.autoMerge &&
    ...
    async checkPushEventCompletion(pushEventId) {
    ...
    async mergeExistingPullRequest(job, required) {
        const targetRepo = job.targetRepo;
        const targetToken = await GithubAppService.getInstallationToken(targetRepo.installationId);
    ...
    async recordApplyConflicts(job, targetDir) {
        let hasConflict = false;
        for (const jobFile of job.files) {
    ...
    applySavedResolvedFiles(job, targetDir) {
        let appliedAny = false;
        for (const jobFile of job.files) {
            if (jobFile.mergeResult !== "MERGED" ||
    ...
    async runRealApply(job, options = {}) {
        ...
        try {
            ...
            let hasConflict = false;
            const applyUpdateItems = [];
            for (const jobFile of job.files) {
                const savedResolved = jobFile.mergeResult === "MERGED" &&
                    typeof jobFile.conflictDiff === "string" &&
                    jobFile.conflictDiff.startsWith(RESOLVED_CONTENT_PREFIX)
                    ? jobFile.conflictDiff
                    : null;
                const syncRes = syncSingleFile(jobFile.filePath, baseRef, job.pushEvent.commitSha, targetDir, tempDir, false, // isDryRun = false
                savedResolved);
                if (syncRes.mergeResult === "CONFLICT") {
                    hasConflict = true;
                }
            ...
            if (hasConflict) {
                throw Object.assign(new Error("Sync job contains merge conflicts. Please resolve conflicts before applying."), { syncStatus: "CONFLICT" });
            }
            ...
            try {
                ...
                if (targetToken)
                    errMsg = errMsg.replaceAll(targetToken, "***");
                if (mainToken)
                    errMsg = errMsg.replaceAll(mainToken, "***");
                if (errMsg.includes("refusing to allow a GitHub App to create or update workflow") &&
                    errMsg.includes("workflows")) {
                    throw new Error(`Git push rejected: The sync modified a GitHub Actions workflow file, but your GitHub App installation is missing the 'Workflows' read/write permission. Please grant the 'Workflows' permission to your GitHub App in its developer settings on GitHub, or exclude workflow files from this sync.\n\nDetails: ${errMsg.trim()}`);
                }
                throw new Error(`Git push failed: ${errMsg.trim()}`);
            }
            // 10. Open Pull Request via GitHub REST API
            const fileListMarkdown = job.files.map((f) => `- \`${f.filePath}\``).join("\n");
            ...
            if (shouldMerge) {
                try {
                    await this.mergeExistingPullRequest({ ...job, prNumber, branchName, targetRepo }, Boolean(options.autoMerge));
                }
                catch (mergeErr) {
