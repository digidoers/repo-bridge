...
export class GithubAppService {
    static async getGithubSettings() {
        return AppSettingsService.getGithubSettings();
    }
    static async getAppJwt() {
    ...
    static async isMockMode() {
        const settings = await this.getGithubSettings();
        return !settings.appId || !settings.privateKey;
    }
    static async getAppInstallUrl() {
        const settings = await this.getGithubSettings();
        if (!settings.appSlug) {
            return null;
        }
     ...
     */
    static async getInstallableRepositories() {
     ...
     */
    static async getAccountRepositories(userToken) {
        const installedRepos = await this.getInstallableRepositories();
        const installedByFullName = new Map(installedRepos.map((repo) => [repo.fullName.toLowerCase(), repo]));
        const repos = [];
        let page = 1;
        while (page <= 10) {
            const reposRes = await fetch(`https://api.github.com/user/repos?affiliation=owner,collaborator,organization_member&visibility=all&sort=updated&per_page=100&page=${page}`, {
                headers: {
    ...
    static async listAccountBranches(userToken, fullName) {
        const branches = [];
        let page = 1;
        while (page <= 10) {
            const res = await fetch(`https://api.github.com/repos/${fullName}/branches?per_page=100&page=${page}`, {
                headers: {
                    Authorization: `Bearer ${userToken}`,
                    Accept: "application/vnd.github+json",
                    "User-Agent": "RepoSync-App",
     ...
     */
    static async verifyInstallation(owner, repo) {
        if (await this.isMockMode()) {
            return null;
        }
        try {
            const allRepos = await this.getInstallableRepositories();
            const match = allRepos.find((r) => r.githubOwner.toLowerCase() === owner.toLowerCase() &&
    ...
    static async getInstallationToken(installationId) {
        ...
        let tokenRes;
        try {
            tokenRes = await fetch(`https://api.github.com/app/installations/${installationId}/access_tokens`, {
                method: "POST",
                headers: {
                    Authorization: `Bearer ${appJwt}`,
                    Accept: "application/vnd.github+json",
     ...
     */
    static async compareCommits(installationId, fullName, base, head) {
        if (await this.isMockMode()) {
            throw new Error("GitHub App is not configured. Real GitHub compare data is required.");
        }
        const token = await this.getInstallationToken(installationId);
        const compareRes = await fetch(`https://api.github.com/repos/${fullName}/compare/${base}...${head}`, {
            headers: {
    ...
    static async listBranches(installationId, fullName) {
        const token = await this.getInstallationToken(installationId);
        const branches = [];
        let page = 1;
        while (page <= 10) {
    ...
    static async listCommits(installationId, fullName, branch, options = {}) {
        const page = Math.max(1, options.page || 1);
        const pageSize = Math.min(100, Math.max(10, options.pageSize || 20));
        const token = await this.getInstallationToken(installationId);
        const res = await fetch(`https://api.github.com/repos/${fullName}/commits?sha=${encodeURIComponent(branch)}&per_page=${pageSize}&page=${page}`, {
            headers: {
                Authorization: `Bearer ${token}`,
                Accept: "application/vnd.github+json",
                "User-Agent": "RepoSync-App",
        ...
        const commits = data.map((commit) => ({
            ...
            if (!search)
                return true;
            return (commit.sha.toLowerCase().includes(search) ||
    ...
    static async getCommit(installationId, fullName, sha) {
        const token = await this.getInstallationToken(installationId);
        const res = await fetch(`https://api.github.com/repos/${fullName}/commits/${encodeURIComponent(sha)}`, {
            headers: {
                Authorization: `Bearer ${token}`,
                Accept: "application/vnd.github+json",
                "User-Agent": "RepoSync-App",
