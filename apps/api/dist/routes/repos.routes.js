...
// ─────────────────────────────────────────────────────────
reposRouter.get("/installable", async (req, res, next) => {
    try {
        ...
        const filtered = installable.filter((r) => !registeredNames.has(r.fullName.toLowerCase()));
...
// ─────────────────────────────────────────────────────────
reposRouter.get("/github-setup", async (req, res, next) => {
    try {
...
// ─────────────────────────────────────────────────────────
reposRouter.get("/github-account/:owner/:repo/branches", async (req, res, next) => {
    try {
        const user = await prisma.user.findUnique({
            where: { id: req.user.id },
            select: { githubToken: true },
...
// ─────────────────────────────────────────────────────────
reposRouter.get("/github-account", async (req, res, next) => {
    try {
        const user = await prisma.user.findUnique({
            where: { id: req.user.id },
            select: { githubToken: true },
...
// POST /repos
// ─────────────────────────────────────────────────────────
reposRouter.post("/", validate(registerRepoSchema), async (req, res, next) => {
    try {
        const { githubOwner, githubName, role, branch, description, customerName, autoMergeEnabled, } = req.body;
        ...
        const installationId = await GithubAppService.verifyInstallation(githubOwner, githubName);
        if (installationId === null) {
            throw AppError.badRequest(`GitHub App is not installed on repository ${githubOwner}/${githubName}. Please install the App first.`);
        }
...
// ─────────────────────────────────────────────────────────
reposRouter.get("/:id/branches", async (req, res, next) => {
    try {
        ...
        const branches = await GithubAppService.listBranches(repo.installationId, repo.fullName);
        res.json({ ok: true, data: branches });
    }
    catch (err) {
        next(err);
...
// ─────────────────────────────────────────────────────────
reposRouter.get("/:id/commits", async (req, res, next) => {
    try {
        ...
        const commits = await GithubAppService.listCommits(repo.installationId, repo.fullName, branch || repo.branch, { page, pageSize, search });
...
// ─────────────────────────────────────────────────────────
reposRouter.get("/:id/commits/:sha/files", async (req, res, next) => {
    try {
        const repo = await prisma.repository.findUnique({
            where: { id: req.params.id },
        });
        ...
        const commit = await GithubAppService.getCommit(repo.installationId, repo.fullName, req.params.sha);
...
// ─────────────────────────────────────────────────────────
reposRouter.get("/:id/compare", async (req, res, next) => {
    try {
        const repo = await prisma.repository.findUnique({
            where: { id: req.params.id },
        });
        ...
        const comparison = await GithubAppService.compareCommits(repo.installationId, repo.fullName, base, head);
...
// PATCH /repos/:id
// ─────────────────────────────────────────────────────────
reposRouter.patch("/:id", validate(updateRepoSchema), async (req, res, next) => {
    try {
        const { branch, description, customerName, isActive, autoMergeEnabled } = req.body;
        // 1. Verify existence
...
//# sourceMappingURL=repos.routes.js.map